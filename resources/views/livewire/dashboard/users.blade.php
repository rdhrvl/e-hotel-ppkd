<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        ...
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <button class="inline-flex items-center gap-1.5 rounded-[var(--radius-sm)] bg-[var(--accent-primary)] hover:bg-[var(--accent-primary-hover)] px-4 py-2 text-xs font-semibold text-white transition-all cursor-pointer" wire:click="openAddModal" id="btn-add-user">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <thead>
                        <tr class="sticky top-0 z-10 select-none">
                            <th class="px-4 py-3 text-xs font-semibold text-[var(--text-muted)] bg-[var(--bg-secondary)] border-b-2 border-[var(--border-color)] cursor-pointer hover:bg-[var(--bg-card-hover)] transition-colors" wire:click="sortBy('name')">
                                User @if($sortField === 'name') {{$sortDirection === 'asc' ? '▲' : '▼'}} @endif
                            </th>
                            ...
                            <th class="px-4 py-3 text-xs font-semibold text-[var(--text-muted)] bg-[var(--bg-secondary)] border-b-2 border-[var(--border-color)]">Role</th>
                    ...
                    <tbody class="divide-y divide-[var(--border-color)] text-xs text-[var(--text-secondary)]">
                        @foreach($users as $user)
                            ...
                            <tr class="hover:bg-[var(--bg-card-hover)] transition-colors even:bg-[var(--bg-primary)]/50">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="font-bold text-[var(--text-primary)] flex items-center gap-1.5">
                                                {{ $user->name }}
                                                @if($user->id === auth()->id())
                                                    <span class="inline-flex items-center rounded-[var(--radius-sm)] px-1.5 py-0.5 text-xs font-semibold border border-[var(--border-color)] bg-[var(--success-bg)] text-[var(--success)]">You</span>
                                                @endif
                                            </div>
                                ...
                                <td class="px-4 py-3.5 text-sm border-b border-[var(--border-color)] font-mono text-[var(--text-muted)]">{{ $user->phone }}</td>
                                <td class="px-4 py-3.5 text-sm border-b border-[var(--border-color)] text-[var(--text-secondary)]">{{ $user->email }}</td>
                                <td class="px-4 py-3.5 text-sm border-b border-[var(--border-color)]">
                                    @php
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" wire:click.self="closeAddModal">
            <div class="w-full max-w-md rounded-[var(--radius-lg)] bg-[var(--bg-card)] border border-[var(--border-color)] p-6 shadow-lg space-y-5">
                <div class="flex items-center gap-2.5 border-b border-[var(--border-color)] pb-3">
                    ...
                    <h3 class="text-base font-semibold text-[var(--text-primary)]">Add New User</h3>
                </div>
                ...
                <form wire:submit.prevent="createUser" class="space-y-4">
                    ...
                    <div>
                        <label class="text-sm font-medium text-[var(--text-secondary)] mb-1.5 block">Role</label>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" wire:click.self="closeEditModal">
            <div class="w-full max-w-md rounded-[var(--radius-lg)] bg-[var(--bg-card)] border border-[var(--border-color)] p-6 shadow-lg space-y-5">
                <div class="flex items-center gap-2.5 border-b border-[var(--border-color)] pb-3">
                    ...
                    <h3 class="text-base font-semibold text-[var(--text-primary)]">Edit User</h3>
                </div>
